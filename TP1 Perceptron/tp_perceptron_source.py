...
if __name__ == "__main__":
    ...
    matrix = rand_gauss()
    plot_2d(matrix, np.ones(100))

    matrix_bi_gauss = rand_bi_gauss()
    plot_2d(matrix_bi_gauss[0], matrix_bi_gauss[1])

    matrix_clown = rand_clown(n1=1000, n2=1000, sigma1=1, sigma2=1)
    plot_2d(matrix_clown[0], matrix_clown[1])

    matrix_checkers = rand_checkers(n1=1000, n2=1000, sigma=0.1)
    plot_2d(matrix_checkers[0], matrix_checkers[1])

    matrix_checkers_predict = predict(matrix_checkers[0], np.array([1, 2, 3]))
    print(matrix_checkers_predict)

    matrix_checkers_predict_class = predict_class(matrix_checkers[0], np.array([1, 2, 3]))
    print(matrix_checkers_predict_class)

    # count the number of -1 and 1 in matrix_checkers[1]
    print(np.unique(matrix_checkers[1], return_counts=True))